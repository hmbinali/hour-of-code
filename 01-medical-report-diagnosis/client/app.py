...
def authenticate_user(username, password):
    try:
        response = requests.get(
            f"{API_URL}/auth/login", auth=HTTPBasicAuth(username, password)
        )
        return response.status_code, response.json()
    except requests.exceptions.ConnectionError:
        return 503, {"detail": "Server is unavailable. Please try again later."}
...
def upload_report(auth, files):
    try:
        ...
        response = requests.post(
            f"{API_URL}/reports/upload", auth=auth, files=files_data, headers=headers
        )
        return response.status_code, response.json()
    except requests.exceptions.ConnectionError:
        return 503, {"detail": "Server is unavailable. Please try again later."}
...
def get_diagnosis(auth, doc_id, question):
    try:
        ...
        response = requests.post(
            f"{API_URL}/diagnosis/from_report", auth=auth, data=data
        )
        return response.status_code, response.json()
    except requests.exceptions.ConnectionError:
        return 503, {"detail": "Server is unavailable. Please try again later."}
...
if not st.session_state.logged_in:
    ...
    if st.session_state.role == "patient":
        ...
        if "doc_id" in st.session_state:
            with st.form("diagnosis_form"):
                diagnosis_doc_id = st.text_input(
                    "Enter your Document ID", value=st.session_state.doc_id
                )
                ...
                if diagnosis_submitted:
                    with st.spinner("Generating diagnosis..."):
                        status_code, data = get_diagnosis(
                            st.session_state.auth, diagnosis_doc_id, diagnosis_question
                        )
                        if status_code == 200:
        ...
        with st.form("doctor_form"):
            ...
            if view_submitted:
                with st.spinner(
                    f"Fetching diagnosis records for {patient_name_input}..."
                ):
                    ...
                    if status_code == 200:
                        ...
                        for record in data:
                            ...
                            st.write(
                                f"**Date:** {datetime.datetime.fromtimestamp(record['timestamp']).strftime('%Y-%m-%d %H:%M:%S')}"
                            )
